...
-- El orden es importante, así que no te pongas creativo y ejecútalo tal cual.
-- --------------------------------------------------------------------------
...
CREATE TABLE usuarios (
    ...
    CONSTRAINT uq_usuarios_email_tienda UNIQUE (tienda_id, email) -- El email debe ser único POR tienda.
);
...
CREATE TABLE productos (
    ...
    descripcion         CLOB, -- CLOB para descripciones largas, por si nos ponemos poéticos.
    ...
    CONSTRAINT chk_productos_precio CHECK (precio > 0), -- No vendemos cosas gratis, esto es un negocio 🤑.
...
-- Esta tabla resuelve la relación "Muchos a Muchos". Un usuario puede tener
-- varios roles, y un rol puede tener muchos usuarios. Sin ella, tendríamos un enredo.
...
-- La misma lógica que usuario_roles. Un producto puede estar en varias categorías.
...
-- Tabla PEDIDOS.
-- LA TABLA MÁS IMPORTANTE. Por eso la particionamos.
-- Particionarla por rango de fecha es como organizar un archivador por año.
-- Cuando busquemos un pedido de 2025, Oracle irá directo a la carpeta "2025".
-- Inteligente, ¿no? 🧐
...
-- Esta tabla es el detalle de la boleta. Cada producto en un pedido es una fila aquí.
-- Así, el pedido y sus detalles siempre estarán juntos. 💕
...
-- Tabla PAGOS.
-- La misma lógica que DETALLES_PEDIDO.
-- Si el pedido está en la partición de 2025, su pago también.
CREATE TABLE pagos (
...
-- DATA WAREHOUSE
-- Aquí es donde los datos se convierten en GOLD 💰 
...
PROMPT FACTOSSS DE VENTAS CREADOS CON ÉXITO 🤑
...
CREATE TABLE tabla_auditoria (
    ...
    valores_nuevos      CLOB,                 -- Valores de la fila después del cambio (para INSERT/UPDATE)
    usuario_accion      VARCHAR2(100) DEFAULT USER NOT NULL, -- Usuario que realizó la acción
